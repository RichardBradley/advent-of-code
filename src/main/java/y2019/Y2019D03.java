...
public class Y2019D03 {

    public static void main(String[] args) throws Exception {
        Stopwatch sw = Stopwatch.createStarted();

        // 1
        assertThat(getClosestCrossingDist(example0)).isEqualTo(6);
        assertThat(getClosestCrossingDist(example1)).isEqualTo(159);
        assertThat(getClosestCrossingDist(example2)).isEqualTo(135);
        System.out.println(getClosestCrossingDist(input));

        // 2
        assertThat(getMinSignalDelay(example0)).isEqualTo(30);
        assertThat(getMinSignalDelay(example1)).isEqualTo(610);
        assertThat(getMinSignalDelay(example2)).isEqualTo(410);
        System.out.println(getMinSignalDelay(input));

        System.out.println("Took " + sw.elapsed(TimeUnit.MILLISECONDS) + "ms");
    }

    private static int getClosestCrossingDist(String spec) {
        String[] lines = spec.split("\n");
        checkArgument(lines.length == 2);
        Set<Point> visited = new HashSet<>();
        for (Point p : followMoves(lines[0])) {
            visited.add(p);
        }
        int minDist = Integer.MAX_VALUE;
        for (Point p : followMoves(lines[1])) {
    ...
    private static int getMinSignalDelay(String spec) {
        String[] lines = spec.split("\n");
        checkArgument(lines.length == 2);
        Map<Point, Integer> visited = new HashMap<>();
        int dist1 = 0;
        for (Point p : followMoves(lines[0])) {
            visited.merge(p, ++dist1, Math::min);
        }
        int minDist = Integer.MAX_VALUE;
        int dist2 = 0;
        for (Point p : followMoves(lines[1])) {
    ...
    private static List<Point> followMoves(String spec) {
        List<Point> acc = new ArrayList<>();
        int x = 0;
        int y = 0;
        for (String move : spec.split(",")) {
            char dirC = move.charAt(0);
