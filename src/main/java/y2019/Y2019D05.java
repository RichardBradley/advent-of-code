...
import static y2019.Y2019D05.OP.*;
...
public class Y2019D05 {
    ...
    private static void eval(int[] program, int input) {
        program = program.clone();
        boolean inputUsed = false;
        int pc = 0;
        outer:
        while (true) {
            Instruction i = parseInstr(program, pc);
            ...
            switch (i.opcode) {
                ...
                case INPUT:
                    checkState(i.size == 2);
                    checkState(!inputUsed);
                    checkState(!i.param1IsImmediate);
                    program[i.param1] = input;
                    inputUsed = true;
                    break;
                case OUTPUT:
                    checkState(i.size == 2);
                    System.out.println(i.param1IsImmediate
                            ? i.param1
                            : program[i.param1]);
                    break;
                default:
