...
public class Y2017D02 {
    ...
    private static int[][] input = parse("1640\t590\t93\t958\t73\t1263\t1405\t1363\t737\t712\t1501\t390\t68\t1554\t959\t79\n" +
    ...
    private static int[][] parse(String spec) {
        String[] inLines = spec.split("\n");
        int[][] outLines = new int[inLines.length][];
        for (int i = 0; i < inLines.length; i++) {
            String[] line = inLines[i].split("\t");
            outLines[i] = new int[line.length];
            for (int j = 0; j < line.length; j++) {
                outLines[i][j] = Integer.parseInt(line[j]);
            }
        }
        return outLines;
    }
