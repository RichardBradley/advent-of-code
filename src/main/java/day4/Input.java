...
public class Input {
    ...
    private static Pattern lineRegex = Pattern.compile("\\[1518-(\\d+-\\d+) (\\d+):(\\d+)] (Guard #(\\d+) begins shift|falls asleep|wakes up)");

    public static List<DayLog> input = parse(lines);
    public static List<DayLog> testInput = parse(TestLines);


    private static List<DayLog> parse(String[] logs) {
        List<DayLog> acc = new ArrayList<>();
        DayLog dayLog = null;
        int asleepStartMin = -1;

        for (String log : logs) {
            Matcher m = lineRegex.matcher(log);
            Preconditions.checkArgument(m.matches());

            String descr = m.group(4);
            if (descr.startsWith("Guard")) {
                checkState(asleepStartMin == -1);
                dayLog = new DayLog();
                dayLog.guardId = Integer.parseInt(m.group(5));
                dayLog.isAsleepByMin = new boolean[60];
                acc.add(dayLog);
                asleepStartMin = -1;
            } else if (descr.startsWith("falls asleep")) {
                checkState(asleepStartMin == -1);
                asleepStartMin = Integer.parseInt(m.group(3));
            } else if (descr.startsWith("wakes up")) {
                checkState(asleepStartMin >= 0);

                int sleepEndMin = Integer.parseInt(m.group(3));
                for (int i = asleepStartMin; i < sleepEndMin; i++) {
                    dayLog.isAsleepByMin[i] = true;
                    dayLog.minsAsleep++;
                }
                asleepStartMin = -1;
            } else {
