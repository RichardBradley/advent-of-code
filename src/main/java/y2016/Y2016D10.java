...
public class Y2016D10 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        assertThat(exampleBotState.whoCompares(5, 2)).isEqualTo(2);

        BotState botState = eval(input);
        System.out.println(botState.whoCompares(61, 17));

        // 2
        System.out.println(botState.outputs.get(0).value * botState.outputs.get(1).value * botState.outputs.get(2).value);
    ...
    static class BotState {
        ...
        Map<Integer, OutputBin> outputs;

        public int whoCompares(int higher, int lower) {
            List<Bot> collect = bots.values().stream()
                    .filter(b -> b.highInput == higher && b.lowInput == lower)
                    .collect(Collectors.toList());
            return Iterables.getOnlyElement(collect).id;
        }
    }

    @Data
    static class Bot implements ChipTarget {
        public Bot(int id) {
            this.id = id;
        }

        int id;
        ChipTarget giveLowTo;
        ChipTarget giveHighTo;
        Integer highInput;
        Integer lowInput;
         ...
         */
        public boolean receiveInput(int input) {
            if (Objects.equals(input, highInput) || Objects.equals(input, lowInput)) {
                return false;
            }
            ...
            if (highInput == null) {
                highInput = input;
                return true;
            } else if (lowInput == null) {
                if (input < highInput) {
                    lowInput = input;
                    return true;
                } else if (input > highInput) {
                    lowInput = highInput;
                    highInput = input;
                    return true;
                } else {
                ...
                throw new IllegalArgumentException(String.format(
                        "bot %s received input %s, but already has highInput=%s lowInput=%s",
                        id,
                        input,
                        highInput,
                        lowInput));
    ...
    @Data
    static class OutputBin implements ChipTarget {
        int id;
        Integer value;
    ...
    interface ChipTarget {
        boolean receiveInput(int input);
    }

    private static BotState eval(String... instructions) {
        Map<Integer, Bot> bots = new TreeMap<>();
        Map<Integer, OutputBin> outputs = new TreeMap<>();
        Pattern givePat = Pattern.compile("bot (\\d+) gives low to (?:output (\\d+)|bot (\\d+)) and high to (?:output (\\d+)|bot (\\d+))");
        Pattern valueInputPat = Pattern.compile("value (\\d+) goes to bot (\\d+)");

        for (String instr : instructions) {
            Matcher giveMatcher = givePat.matcher(instr);
            if (giveMatcher.matches()) {
                int fromId = Integer.parseInt(giveMatcher.group(1));

                Bot from = bots.computeIfAbsent(fromId, id -> new Bot(id));
                checkState(from.giveLowTo == null);
                checkState(from.giveHighTo == null);

                if (null != giveMatcher.group(2)) {
                    int lowOutputId = Integer.parseInt(giveMatcher.group(2));
                    OutputBin lowOutput = outputs.computeIfAbsent(lowOutputId, id -> new OutputBin(id));
                    from.giveLowTo = lowOutput;
                } else {
                    int lowBotId = Integer.parseInt(giveMatcher.group(3));
                    Bot lowBot = bots.computeIfAbsent(lowBotId, id -> new Bot(id));
                    from.giveLowTo = lowBot;
                }
                if (null != giveMatcher.group(4)) {
                    int highOutputId = Integer.parseInt(giveMatcher.group(4));
                    OutputBin highOutput = outputs.computeIfAbsent(highOutputId, id -> new OutputBin(id));
                    from.giveHighTo = highOutput;
                } else {
                    int highBotId = Integer.parseInt(giveMatcher.group(5));
                    Bot highBot = bots.computeIfAbsent(highBotId, id -> new Bot(id));
                    from.giveHighTo = highBot;
                }
            } else {
                Matcher valueInputMatcher = valueInputPat.matcher(instr);
                if (valueInputMatcher.matches()) {
                    int inputVal = Integer.parseInt(valueInputMatcher.group(1));
                    int toId = Integer.parseInt(valueInputMatcher.group(2));

                    Bot to = bots.computeIfAbsent(toId, id -> new Bot(id));
                    to.receiveInput(inputVal);
                } else {
        ...
        // all instructions set up, now pump the system until equilibrium
        boolean changesMade;
        int scanCount = 0;
        do {
            System.out.println("Scanning bots " + (++scanCount));
            changesMade = false;
            Queue<Bot> queue = new ArrayDeque<>(bots.values());

            Bot bot;
            while (null != (bot = queue.poll())) {
                if (bot.highInput != null && bot.lowInput != null) {
                    checkNotNull(bot.giveHighTo);
                    checkNotNull(bot.giveLowTo);
                    if ((bot.giveHighTo).receiveInput(bot.highInput)) {
                        changesMade = true;
                        if (bot.giveHighTo instanceof Bot) {
                            queue.add(((Bot) bot.giveHighTo));
                        }
                    }
                    if ((bot.giveLowTo).receiveInput(bot.lowInput)) {
                        changesMade = true;
                        if (bot.giveLowTo instanceof Bot) {
                            queue.add(((Bot) bot.giveLowTo));
                        }
                    }
                }
            }
        ...
        return new BotState(bots, outputs);
    }
