...
public class Y2016D04 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 1
        assertThat(parseRoom("aaaaa-bbb-z-y-x-123[abxyz]")).isNotNull();
        assertThat(parseRoom("a-b-c-d-e-f-g-h-987[abcde]")).isNotNull();
        assertThat(parseRoom("not-a-real-room-404[oarel]")).isNotNull();
        assertThat(parseRoom("totally-real-room-200[decoy]")).isNull();

        System.out.println((Integer) Arrays.stream(input)
                .map(x -> parseRoom(x))
                .filter(x -> null != x)
                .mapToInt(x -> x.sectorId)
                .sum());

        // 2
        for (String i : input) {
            Room room = parseRoom(i);
            if (room != null) {
                System.out.println(decryptName(room) + "\t" + room.sectorId);
            }
        }
    ...
    private static String decryptName(Room room) {
        ...
        for (char c : room.encryptedName.toCharArray()) {
            if (c == '-') {
                acc.append(' ');
            } else {
                acc.append((char) ('a' + (((c - 'a') + room.sectorId) % 26)));
            }
        }
        return acc.toString();
    }
    ...
    static Pattern ROOM_PAT = Pattern.compile("([a-z-]+)-(\\d+)\\[([a-z]+)]");

    private static Room parseRoom(String room) {
        Matcher matcher = ROOM_PAT.matcher(room);
        Preconditions.checkArgument(matcher.matches());
        String encryptedName = matcher.group(1);
        int sectorId = Integer.parseInt(matcher.group(2));
        String checksum = matcher.group(3);

        Map<Integer, List<Integer>> collect = encryptedName.chars()
                .filter(c -> c != '-')
                .boxed()
                .collect(Collectors.groupingBy(c -> c));
        String expectedChecksum = collect.entrySet().stream()
                .sorted(Comparator.comparing((Map.Entry<Integer, List<Integer>> x) -> -x.getValue().size())
                        .thenComparing(x -> x.getKey()))
                .limit(5)
                .map(x -> Character.toString((char) (int) x.getKey()))
