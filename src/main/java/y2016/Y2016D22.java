...
public class Y2016D22 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 1
        System.out.println(countViablePairs(parse(input)));

        // 2
        assertThat(findShortestRouteLen(parse(exampleInput))).isEqualTo(7);
        System.out.println("example ok");

        // qq print the map, look by hand
        System.out.println(findShortestRouteLen(parse(input)));
    ...
    static class SearchState implements Comparable<SearchState> {
        ...
        int width;
        int height;
        Point goalDataLocation;
        private final int searchHeuristicCost;

        public SearchState(int movesToHere, int[] used, int[] sizes, int width, int height, Point goalDataLocation) {
            this.movesToHere = movesToHere;
            this.used = used;
            this.sizes = sizes;
            this.width = width;
            this.height = height;
            this.goalDataLocation = goalDataLocation;

            this.searchHeuristicCost = movesToHere + minMovesToFinish(goalDataLocation);
        }

        public static SearchState from(List<NodeInfo> world) {
            int width = 0;
            int height = 0;
            ...
            for (NodeInfo nodeInfo : world) {
                used[nodeInfo.x * height + nodeInfo.y] = nodeInfo.usedTB;
                sizes[nodeInfo.x * height + nodeInfo.y] = nodeInfo.sizeTB;
            }
        ...
        private int minMovesToFinish(Point goalDataLocation) {
            // To be admissable, must not over-estimate the dist to finish
            return goalDataLocation.x + goalDataLocation.y;
        }
        ...
        public Collection<SearchState> possibleMoves() {
            ...
            for (int fromX = 0; fromX < width; fromX++) {
                for (int fromY = 0; fromY < height; fromY++) {
                    int fromIdx = fromX * height + fromY;
                    boolean isGoalData = fromX == goalDataLocation.x && fromY == goalDataLocation.y;
                    // copy up?
                    if (fromY > 0) {
                        int toIdx = fromX * height + (fromY - 1);
                        if (sizes[toIdx] >= used[fromIdx] + used[toIdx]) {
                            int[] newUsed = used.clone();
                            newUsed[toIdx] += newUsed[fromIdx];
                            newUsed[fromIdx] = 0;
                    ...
                    // copy left
                    if (fromX > 0) {
                        int toIdx = (fromX - 1) * height + fromY;
                        if (sizes[toIdx] >= used[fromIdx] + used[toIdx]) {
                            int[] newUsed = used.clone();
                            newUsed[toIdx] += newUsed[fromIdx];
                            newUsed[fromIdx] = 0;
    ...
    private static int findShortestRouteLen(List<NodeInfo> world) {
        ...
        queue.add(SearchState.from(world));

        while (true) {
            SearchState state = queue.poll();

            if (state.goalDataLocation.equals(target)) {
    ...
    static String input = "root@ebhq-gridcenter# df -h\n" +
