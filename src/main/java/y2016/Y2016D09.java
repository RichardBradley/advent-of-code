...
public class Y2016D09 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 2
        assertThat(decompress2Len("ADVENT")).isEqualTo(6);
        assertThat(decompress2Len("(3x3)XYZ")).isEqualTo(9);
        assertThat(decompress2Len("X(8x2)(3x3)ABCY")).isEqualTo("XABCABCABCABCABCABCY".length());
        assertThat(decompress2Len("(27x12)(20x12)(13x14)(7x10)(1x12)A")).isEqualTo(241920);
        assertThat(decompress2Len("(25x3)(3x3)ABC(2x3)XY(5x2)PQRSTX(18x9)(3x2)TWO(5x7)SEVEN")).isEqualTo(445);

        System.out.println(decompress2Len(input));

        System.out.println("Took " + sw.elapsed(TimeUnit.MILLISECONDS) + "ms");
    }

    private static long decompress2Len(String input) throws IOException {
        ...
        while (-1 != (c = in.read())) {
            if (c == '(') {
                ...
                int next;
                while (Character.isDigit(next = in.read())) {
                    lenS.append((char) next);
                }
                checkState(next == 'x');
                int len = Integer.parseInt(lenS.toString());

                StringBuilder repS = new StringBuilder();
                while (Character.isDigit(next = in.read())) {
                    repS.append((char) next);
                }
                checkState(next == ')');
                ...
                for (int i = 0; i < len; i++) {
                    repeated.append((char) in.read());
                }
                long repeatedSectionLen = decompress2Len(repeated.toString());
                length += rep * repeatedSectionLen;
            } else {
                length++;
            }
        }
    ...
    private static String decompress(String input) throws Exception {
        Reader in = new StringReader(input);
