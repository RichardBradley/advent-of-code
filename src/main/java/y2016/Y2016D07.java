...
public class Y2016D07 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(Arrays.stream(input).filter(Y2016D07::supportsTls).count());
        ...
        assertThat(supportsSsl("zazbz[bzb]cdb")).isTrue();

        // 354 too high
        System.out.println(Arrays.stream(input).filter(Y2016D07::supportsSsl).count());
    ...
    private static boolean supportsTls(String address) {
        ...
        for (int i = 0; i < address.length(); i++) {
            ...
            switch (c) {
                case '[':
                    Preconditions.checkState(!inBrackets);
                    inBrackets = true;
                    startOfSectionIdx = i + 1;
                    break;
                case ']':
                    Preconditions.checkState(inBrackets);
                    inBrackets = false;
                ...
                default:
                    // check for ABBA
                    if (i - startOfSectionIdx >= 3) {
                        if (address.charAt(i - 3) == c
                                && address.charAt(i - 2) == address.charAt(i - 1)
                                && address.charAt(i - 1) != c) {
    ...
    private static boolean supportsSsl(String address) {
        boolean inBrackets = false;
        List<StringBuilder> hypernets = new ArrayList<>();
        List<String> expectedBabs = new ArrayList<>();
        StringBuilder currentHypernet = null;
        int startOfSectionIdx = 0;
        ...
        for (int i = 0; i < address.length(); i++) {
            ...
            switch (c) {
                case '[':
                    ...
                    inBrackets = true;
                    startOfSectionIdx = i + 1;
                    hypernets.add(currentHypernet = new StringBuilder());
                    break;
                case ']':
                    Preconditions.checkState(inBrackets);
                    inBrackets = false;
                ...
                default:
                    if (inBrackets) {
                        currentHypernet.append(c);
                    } else {

                        // check for ABA
                        if (i - startOfSectionIdx >= 2) {
                            if (address.charAt(i - 2) == c
                                    && address.charAt(i - 1) != c) {

                                char oppC = address.charAt(i - 1);
                                expectedBabs.add("" + oppC + c + oppC);
                            }
                        }
                    }
            }
        ...
        for (String expectedBab : expectedBabs) {
            for (StringBuilder hypernet : hypernets) {
                if (hypernet.indexOf(expectedBab) >= 0) {
                    return true;
                }
            }
        }
