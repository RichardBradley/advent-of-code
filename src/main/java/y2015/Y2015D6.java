...
public class Y2015D6 {
    public static void main(String[] args) throws Exception {
        ...
        // 1
        assertThat(countLit("turn on 0,0 through 999,999")).isEqualTo(1000000);
        assertThat(countLit("turn on 0,0 through 999,999", "toggle 0,0 through 999,0")).isEqualTo(1000000 - 1000);
        assertThat(countLit("turn on 0,0 through 999,999", "turn off 499,499 through 500,500")).isEqualTo(1000000 - 4);
        assertThat(countLit("turn on 0,0 through 999,999", "toggle 0,0 through 999,0", "toggle 0,0 through 9,0")).isEqualTo(1000000 - 1000 + 10);

        System.out.println(countLit(input));

        // 2
        assertThat(countLit2("turn on 0,0 through 0,0")).isEqualTo(1);
        assertThat(countLit2("toggle 0,0 through 999,999")).isEqualTo(2000000);

        System.out.println(countLit2(input));
    }

    private static int countLit(String... lines) {
        int size = 1000;
        boolean[][] lights = new boolean[size][];
        ...
        for (String line : lines) {
            ...
            for (int x = x1; x <= x2; x++) {
                for (int y = y1; y <= y2; y++) {
                    if ("turn on".equals(m.group(1))) {
                        lights[x][y] = true;
                    } else if ("turn off".equals(m.group(1))) {
                        lights[x][y] = false;
                    } else {
                        lights[x][y] ^= true;
                    }
    ...
    private static int countLit2(String... lines) {
        int size = 1000;
