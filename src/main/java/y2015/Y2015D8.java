...
public class Y2015D8 {
    public static void main(String[] args) throws Exception {
        ...
        // 1
        assertThat(diffCodeMemory("\"\"")).isEqualTo(2 - 0);
        assertThat(diffCodeMemory("\"abc\"")).isEqualTo(5 - 3);
        assertThat(diffCodeMemory("\"aaa\\\"aaa\"")).isEqualTo(10 - 7);
        assertThat(diffCodeMemory("\"\\x27\"")).isEqualTo(6 - 1);

        System.out.println(
                (Integer) Arrays.stream(input.split("\n")).mapToInt(Y2015D8::diffCodeMemory).sum());

        // 2
        assertThat(diffCodeRecoded("\"\"")).isEqualTo(6 - 2);
        assertThat(diffCodeRecoded("\"abc\"")).isEqualTo(9 - 5);
        assertThat(diffCodeRecoded("\"aaa\\\"aaa\"")).isEqualTo(16 - 10);
        assertThat(diffCodeRecoded("\"\\x27\"")).isEqualTo(11 - 6);

        System.out.println(
                (Integer) Arrays.stream(input.split("\n")).mapToInt(Y2015D8::diffCodeRecoded).sum());
    }

    private static int diffCodeMemory(String code) {
        checkArgument(code.charAt(0) == '"');
        checkArgument(code.charAt(code.length() - 1) == '"');
        int memLen = 0;

        for (int i = 1; i < code.length() - 1; i++) {
            if (code.charAt(i) == '\\') {
                checkArgument(i < code.length() - 2);
                if (code.charAt(i + 1) == '\\') {
                    i++;
                    memLen++;
                    ...
                    checkArgument(code.charAt(i + 1) == 'x');
                    checkArgument(isHexDigit(code.charAt(i + 2)));
                    checkArgument(isHexDigit(code.charAt(i + 3)));
                    i += 3;
                    memLen++;
                }
            } else {
        ...
        return code.length() - memLen;
    }

    private static int diffCodeRecoded(String code) {
        int recodedLen = 2;
    ...
    private static String input = "\"\\xa8br\\x8bjr\\\"\"\n" +
            "\"nq\"\n" +
